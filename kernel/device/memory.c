...
void mem_pool_init(uint32_t all_mem){
    ...
    uint32_t page_table_size=PG_SIZE*256;  // 页表大小，256个页框

    uint32_t used_mem=page_table_size+0x100000;
    uint32_t free_mem=all_mem-used_mem;
    uint16_t all_free_pages=free_mem/PG_SIZE; //有多少个页？

    uint16_t kernel_free_pages=all_free_pages/2; //将剩余的一半分给内核
    ...
    user_pool.pool_bitmap.bits=(void*)(MEM_BITMAP_BASE+kbm_length);//kmp_length是内核内存池的长度,单位是字节
...
void mem_init(){
    ...
    uint32_t mem_bytes_total=(*(uint32_t*)(0x900));
    put_str("mem_size_is: 0x");
